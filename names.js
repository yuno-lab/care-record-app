...
function renderNames() {
  ...
  document.querySelectorAll('.delete-name-btn').forEach((button) => {
    button.addEventListener('click', (e) => deleteName(Number(e.target.dataset.index)));
  });
}

function deleteName(index) {
  if (!window.confirm('この対象者名を削除してよろしいですか？')) return;
  const names = getNames();
...
if (nameForm) {
  nameForm.addEventListener('submit', (e) => {
    ...
    const names = getNames();
    if (names.some((item) => item.name === newName)) return alert('同じ名前はすでに登録されています。');
    names.push({ id: `n_${Date.now()}_${Math.floor(Math.random() * 100000)}`, name: newName });
    saveNames(names);
